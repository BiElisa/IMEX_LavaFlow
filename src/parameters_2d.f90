...
MODULE parameters_2d
  ...
  ! EB : added vertical profile flags
  
  !> Flag to choose if we model the parabolic velocity profile
  !> - T      => parabolic velocity profile
  !> - F      => constant velocity profile
  !> .
  LOGICAL :: velocity_profile_flag
  
  !> Flag to choose if we model a piecewise linear temperature profile
  !> - T      => piece-wise linear temperature profile
  !> - F      => constant temperature profile
  !> .
  LOGICAL :: temperature_profile_flag
  
  ! EB : end (added vertical profile flags)
